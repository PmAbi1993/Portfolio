...
export function Window({ win, children }) {
    ...
    if (win.minimized)
        return null;
    return (_jsxs("div", { ref: ref, role: "dialog", "aria-label": win.title, className: "fixed bg-white/80 dark:bg-black/60 backdrop-blur-md text-white rounded-mac shadow-mac border border-white/10 overflow-hidden", style: { left: win.rect.x, top: win.rect.y, width: win.rect.width, height: win.rect.height, zIndex: win.z }, onMouseDown: () => focusWindow(win.id), children: [_jsxs("div", { className: "h-8 flex items-center gap-2 px-3", onMouseDown: (e) => {
